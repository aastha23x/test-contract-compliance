...
def fetch_all_data():
    ...
    The dashboard reads the full history separately for the events table.
    ...
    # Historical events stay in webhook_data.json for the dashboard

    # events table, but the compliance scan only looks at what just changed.
    ...
    total = sum(len(v) for v in {repo_name: [payload]}.values())
...

{
  ...
  "violations": [

    {
      ...
      "description": "<DETAILED_EXPLANATION_2_3_PARAGRAPHS>",

      "remediation": "<STEP_BY_STEP_FIX>"
